...
ROOT_DIR = 'C:\\Users\\heegyoon\\Desktop\\data\\IAS\\vn\\raw\\TNGo_new2'
TMP_DIR = 'C:\\Users\\heegyoon\\Desktop\\data\\IAS\\vn\\tmp\\TNGo_new2'
OUTPUT_DIR = 'C:\\Users\\heegyoon\\Desktop\\data\\IAS\\vn\\dataset\\TNGo_new2'
...
def check_label(data):
    ...
    if label['spoof_label'] == 'Real':
        dirname1 = 'Real'
        dirname2 = unquote(os.path.basename(os.path.dirname(tmp)))
...
def main():
    json_list = ['Real', 'Fake_Laptop', 'Fake_Monitor', 'Fake_Paper', 'Fake_SmartPhone']
    
    json_path_list = [os.path.join(ROOT_DIR, 'label', i+'.json') for i in json_list]
    ...
    for json_path in json_path_list:
        ...
        for d in data:
            ...
            if person_name.endswith(' '):
                person_name = person_name[:-1]
            os.makedirs(os.path.join(TMP_DIR, person_name), exist_ok=True)
            
            src = os.path.join(ROOT_DIR, label['from_path'])
            if os.path.isfile(src) and is_image_file(src):
                dst_filename = make_filename(label)
                dst = os.path.join(TMP_DIR, person_name, dst_filename + os.path.splitext(src)[1])
                json_path = os.path.join(TMP_DIR, person_name, dst_filename + '.json')
    ...
    person_list = os.listdir(TMP_DIR)
    for person in person_list:
        src_person_dir = os.path.join(TMP_DIR, person)
        
        files = os.listdir(src_person_dir)
        if len(files) == 20:
            dst_person_dir = os.path.join(OUTPUT_DIR, person)
